...
export default function ShowTrends() {
    ...
    const hoverVariant = {
        hover: { scale: 1.1, rotate: -5, x: -5, transition: { duration: 0.2, ease: "easeInOut" } },
    ...
    return (
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
            {movies && movies.map((movie) => (
                <li key={movie.id}>
                    <motion.div
                        className="w-full bg-gradient-to-b from-neutral-900 to-black grid grid-cols-12 gap-3 border border-neutral-800 rounded-xl p-3"
                        initial="initial"
                        whileHover="hover"
                    >
                        <motion.div
                        ...
                        <div className="col-span-7 flex flex-col h-full justify-between">
                            <div className="w-full flex gap-2 text-xs justify-end">
                                { movie.release_date && 
                                    <span className="bg-neutral-800 rounded-full px-1.5 py-0.5">
